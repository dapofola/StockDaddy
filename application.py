...
def performances(stock_info, values, numShares, prices):
	...
	mkt_performance = round(100*(sp500['open']/sp500['regularMarketPreviousClose'] - 1),2)
...
def getDashboard(username):
	...
	prices = [round(stock['currentPrice'] if 'currentPrice' in stock.keys() else stock['navPrice'],2) for stock in stock_info]
...
def dashboard():
	...
	if session['userType'] == "novice":
		...
		zagP = round(yf.Ticker('ZAG.TO').info['navPrice'],2)
		vfvP = round(yf.Ticker('VFV.TO').info['navPrice'],2)
		xuuP = round(yf.Ticker('XUU.TO').info['navPrice'],2)
		xefP = round(yf.Ticker('XEF.TO').info['navPrice'],2)
		...
		cashRemaining = round(cash - totalCashSpent,2)
...
def currency():
	...
	req_data = requests.get('http://api.exchangeratesapi.io/v1/latest?access_key=634b658bb091383b0279f0e26f70b479')
...
def trade():
	...
	return render_template('trade.html', cash="{:,}".format(round(query[0][0],2)), flag="false", price=-50)
...
def stockSearch():
	...
	print(yf.Ticker(tickerName).info)

	try:
		price = yf.Ticker(tickerName).info['currentPrice'] if 'currentPrice' in yf.Ticker(tickerName).info.keys() else round(yf.Ticker(tickerName).info['navPrice'],2)
		try:
	...
	return render_template('trade.html', ticker=tickerName.upper(), price=price, cash="{:,}".format(query[0][0],2), name=companyName, quantity=numShares, message=message, flag='true', myList=fList, strDate=strDate, bgColor=bgColor, borderColor=borderColor, vol=vol, beta=beta, mcap=mcap, pe=pe, div=div, eps=eps, so=so)
...
def executeTrade():
	...
	if signal == "buy":
		stockPrice = round(yf.Ticker(session['search']).info['currentPrice'] if 'currentPrice' in yf.Ticker(session['search']).info.keys() else yf.Ticker(session['search']).info['navPrice'],2)
		...
		if value > cashAvail:
			...
			if query == 1:
				...
				cashAvail = round(float(cashAvail) - float(value),2)
		...
		if quantOwned < quant:
			...
			stockPrice = stockPrice = round(yf.Ticker(session['search']).info['currentPrice'] if 'currentPrice' in yf.Ticker(session['search']).info.keys() else yf.Ticker(session['search']).info['navPrice'],2)
...
def portfolio():
	...
	numShares = ["{:,}".format(numShare) for (ticker, numShare, purchasePrice) in holdings]
	stock_info = [yf.Ticker(ticker).info for ticker in tickers]
	names = [stock['longName'] for stock in stock_info]
	names = [name.replace('&amp;', '&') for name in names]
	purchase_price = [round(purchasePrice,2) for (ticker,numShares,purchasePrice) in holdings]
	current_price = [round(decimal.Decimal(stock['currentPrice'] if 'currentPrice' in stock.keys() else stock['navPrice']),2) for stock in stock_info]
	daily_return = [str(round(stock['currentPrice'] if 'currentPrice' in stock.keys() else stock['navPrice']
		/stock['regularMarketPreviousClose'] * 100 - 100,2))+'%' for stock in stock_info]
